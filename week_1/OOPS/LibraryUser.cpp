#include<bits/stdc++.h>
using namespace std;
class LibraryUser{
    public:
        virtual void registerAccount()=0;
        virtual void requestBook()=0;
};
class KidUser: public LibraryUser{
    public:
    int age;
    string bookType;
    KidUser(int age,string bookType){
        this->age=age;
        this->bookType=bookType;
    }
    void registerAccount() override {
        if(age<12){
            cout<<"You have successfully registered under a kids account"<<endl;
        }else{
            cout<<"Sorry age must be less than 12 to register as kid"<<endl;
        }
            
        }
    void requestBook() override {
        if(bookType=="Kids"){
            cout<<"Book issued successfully,please return the book within 10 days"<<endl;
        }else{
            cout<<"Oops,you are allowed to take only kids book"<<endl;
        }
    }
    };
class AdultUser: public LibraryUser{
    public:
    int age;
    string bookType;
    AdultUser(int age,string bookType){
        this->age=age;
        this->bookType=bookType;
    }
    void registerAccount() override {
        if(age>12){
            cout<<"You have successfully registered under an adult account"<<endl;
        }else{
            cout<<"Sorry,Age must be greater than 12 to register under an Adult Account"<<endl;
        }
    }
    void requestBook() override {
        if(bookType=="Fiction"){
            cout<<"Book issued successfully, please return the book within 7 days"<<endl;
        }else{
            cout<<"Oops,you are allowed to take only Adult Fiction books"<<endl;
        }
    }
};
int main(){
    KidUser kid1(10,"Kids");
    kid1.registerAccount();
    kid1.requestBook();

    KidUser kid2(18,"Fiction");
    kid2.registerAccount();
    kid2.requestBook();

    AdultUser Person1(5,"Kids");
    Person1.registerAccount();
    Person1.requestBook();

    AdultUser Person2(23,"Fiction");
    Person2.registerAccount();
    Person2.requestBook();

}